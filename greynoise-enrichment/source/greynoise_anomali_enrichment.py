...
def enrichIP(anomali_enrichment, search_string):
    try:
        ...
        riot_response = requests.get(api_base + '/riot/' + search_string,
                                     headers={"Accept": "application/json", "key": api_key,
                                              "User-Agent": "greynoise-anomali-enrichment-" + version})
        ...
        if response.status_code == 200 and response_json['seen']:
            ...
            table_widget_additional.addRowOfItems([ItemInWidget(itemValue='Tor'),
                                                   ItemInWidget(itemValue=response_json['metadata'].get('tor',
                                                                                                        'unknown') or 'unknown')])
            ...
            table_widget_additional.addRowOfItems([ItemInWidget(itemValue='rDNS'),
                                                   ItemInWidget(itemValue=response_json['metadata'].get('rdns',
                                                                                                        'unknown') or 'unknown')])
            table_widget_additional.addRowOfItems([ItemInWidget(itemValue='OS'),
                                                   ItemInWidget(itemValue=response_json['metadata'].get('os',
                                                                                                        'unknown') or 'unknown')])
            ...
            table_widget_additional.addRowOfItems([ItemInWidget(itemValue='BOT'),
                                                   ItemInWidget(
                                                       itemValue=response_json.get('bot', 'N/A') or 'N/A')])
            ...
            if port_list:
                ...
                if len(response_json['raw_data']['scan']) > 10:
                    port_composite_item.addItemInWidget(ItemInWidget(ItemTypes.String, ('Output limited to 10 of {} '
                                                                                        'items, see Visualizer for more details.').format(
            ...
            if tag_list:
                ...
                if len(response_json['tags']) > 10:
                    tag_composite_item.addItemInWidget(
                        ItemInWidget(ItemTypes.String, ('Output limited to 10 of {} items,'
                                                        ' see Visualizer for more details.').format(
            ...
            if ua_list:
                ...
                if len(response_json['raw_data']['web']['useragents']) > 10:
                    ua_composite_item.addItemInWidget(
                        ItemInWidget(ItemTypes.String, ('Output limited to 10 of {} items, '
                                                        'see Visualizer for more details.').format(
                            str(len(response_json['raw_data']['web']
                                    ['useragents'])))))
            ...
            if ja3_list:
                ...
                table_widget_additional.addRowOfItems(
                    [ItemInWidget(ItemTypes.String, 'JA3(s) (fingerprint/port)'), ja3_composite_item])
            else:
            ...
            if hassh_list:
                ...
                table_widget_additional.addRowOfItems(
                    [ItemInWidget(ItemTypes.String, 'HASSH(s) (fingerprint/port)'), hassh_composite_item])
            else:
            ...
            # Table Widget #1 Start
            table_widget = TableWidget("Details", ["Key", "Value"], columnWidths=['20%', '80%'])
            table_widget.addRowOfItems([ItemInWidget(itemValue='Last Updated'),
            ...
            table_widget.addRowOfItems([ItemInWidget(itemValue='Name'),
                                        ItemInWidget(itemValue=riot_response_json.get('name', 'unknown') or 'unknown')])
            table_widget.addRowOfItems([ItemInWidget(itemValue='Classification'),
                                            ItemInWidget(itemValue='Benign Service',
                                                         backgroundColor="#3CB371", textColor="White")])
            table_widget.addRowOfItems([ItemInWidget(itemValue='Description'),
                                        ItemInWidget(itemValue=riot_response_json['description'])])
            table_widget.addRowOfItems([ItemInWidget(itemValue='Explanation'),
                                        ItemInWidget(itemValue=riot_response_json['explanation'])])
            table_widget.addRowOfItems([ItemInWidget(itemValue='Reference URL'),
